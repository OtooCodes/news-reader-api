...
tags_metadata = [
    ...
    {
        "name": "Saved Articles",
        "description": "Manage your saved articles",
    },
    {
        "name": "Digest",
        "description": "Get your daily news digest",
    },
]
...
def get_daily_digest():
    """Get the last five saved articles as a daily digest"""
    # Get articles from the last 24 hours
    yesterday = datetime.utcnow() - timedelta(days=1)
    
    recent_articles = saved_articles_collection.find({
        "date_saved": {"$gte": yesterday.isoformat()}
    }).sort("date_saved", -1).limit(5).to_list()
...
def delete_saved_article(article_id: str):
    ...
    # Check if article_id is valid mongo id
